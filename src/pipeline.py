...
def run_train_only():
    """Train models only (data processing + training, no evaluation)."""
    MODEL_DIR.mkdir(parents=True, exist_ok=True)
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    ...
    step_5_train_models(X_train, X_test, y_test)
    print("\nTraining complete. Models saved to trained_models/")


def run_evaluate_only():
    """Evaluate pre-trained models using saved test data.

    Note: Uses joblib to load scikit-learn models from trusted local files
    generated by this pipeline's training step.
    """
    ...
    step_6_evaluate(if_model, lof_model, ocsvm_model, X_test, y_test)
    step_7_dynamic_threshold(if_model, X_test, y_test)
    step_8_feature_importance(if_model, X_test, y_test)
